...
const STARTUP_PATTERN =
  /if\((\w+=\w+==="production"\?"":`\$\{\w+\}\/`),process\.platform!=="darwin"&&process\.platform!=="win32"\)(return[\s\S]{0,240}?platform:process\.platform\}\),[!]1);try/;
...
export function patchLinuxNativeUpdaterContent(
  ...
  const withBridge = content.replace(
    "async function iit()",
    `${buildBridgeSource(electron)}async function iit()`,
  );
